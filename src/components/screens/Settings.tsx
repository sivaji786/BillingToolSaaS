...
import { Building2, FileText, Upload, X, Shield, CheckCircle2, Loader2, ExternalLink, Users, ShieldCheck } from 'lucide-react';
...
import { WorkHubSettings } from '../../pages/WorkHub/WorkHubSettings';
import { profileService } from '../../services/workhubApi';
...
export function Settings({ profile, onUpdateProfile }: SettingsProps) {
  ...
  const seesWhSettings = workhubEnabled && (
    whProfile?.is_admin || ['planner', 'manager', 'finance'].includes(whProfile?.role ?? '')
  );

  // Users & Roles sub-navigation state
  type AdminView = 'list' | 'user-form' | 'role-form';
  const [adminView, setAdminView] = useState<AdminView>('list');
  const [editingId, setEditingId] = useState<string | null>(null);
  const companyTypeId = editedProfile.companyTypeId != null ? String(editedProfile.companyTypeId) : null;
  ...
  return (
    <div className="space-y-6">
      ...
      <Tabs value={activeTab} onValueChange={(v) => { setActiveTab(v); setAdminView('list'); setEditingId(null); }} className="space-y-6">
        <TabsList>
        ...
        {/* SSO & SAML — Connected Accounts (all users) + Enterprise SSO config (admins only) */}
        <TabsContent value="sso">
          <div className="space-y-6">
            {/* Connected Accounts — visible to everyone */}
            ...
            {/* Enterprise SSO config — admins only */}
            {isAdmin && (
              <Card className="p-6 space-y-6">
                <div>
            ...
            <Card className="p-6">
              {adminView === 'role-form' ? (
                <RoleForm
                  ...
                  onBack={() => { setAdminView('list'); setEditingId(null); }}
                />
              ) : (
                <RoleList
                  companyTypeId={companyTypeId}
                  onCreate={() => { setEditingId(null); setAdminView('role-form'); }}
                  onEdit={(id) => { setEditingId(id); setAdminView('role-form'); }}
                />
              )}
            </Card>
          </TabsContent>
        ...
        {/* Team tab — Users (admin) + WorkHub settings (privileged workhub users) */}
        {(isAdmin || seesWhSettings) && (
          <TabsContent value="team">
            {/* When a user form is open it takes over the full tab */}
            {adminView === 'user-form' ? (
              <Card className="p-6">
                <UserForm
                  userId={editingId}
              ...
              <div className="space-y-6">
                {isAdmin && (
                  <Card className="p-6">
                    <UserList
                      companyTypeId={companyTypeId}
                      onCreate={() => { setEditingId(null); setAdminView('user-form'); }}
                      onEdit={(id) => { setEditingId(id); setAdminView('user-form'); }}
                    />
                  </Card>
                )}
                {seesWhSettings && <WorkHubSettings />}
              </div>
            )}
          </TabsContent>
        )}
      ...
      {/* Save Button — only relevant for profile/payment/defaults tabs */}
      {!['team', 'roles', 'sso'].includes(activeTab) && (
