...
import { ArrowLeft, Loader2, ExternalLink, Save, Calendar, MessageSquare, User } from 'lucide-react';
...
export function SATicketDetails({ ticketId, onNavigate }: SATicketDetailsProps) {
    ...
    const hasChanges = ticket && (status !== (ticket.status || 'open') || priority !== (ticket.priority || 'medium') || comment.trim() !== '');
            ...
            <Card>
                <CardHeader className="border-b bg-slate-50/50 p-6">
                    <div className="flex flex-col lg:flex-row justify-between gap-6">
                        ...
                        {/* Right Side: Quick Actions / Status Form */}
                        <div className="flex flex-col sm:flex-row lg:flex-col gap-4 min-w-fit">
                            <div className="flex items-center gap-4 bg-white p-2 rounded-2xl border shadow-sm border-slate-200">
                                <div className="flex flex-col px-2">
                                    ...
                                    <Select value={status} onValueChange={setStatus}>
                                        <SelectTrigger className="w-[140px] h-9 border-none bg-transparent hover:bg-slate-50 transition-colors font-semibold p-0 shadow-none focus:ring-0">
                                            <SelectValue />
                                        </SelectTrigger>
                                        <SelectContent>
                            ...
                            {hasChanges && (
                                <div className="flex flex-col gap-3 animate-in fade-in zoom-in-95 duration-300">
                                    ...
                                    <Button
                                        size="lg"
                                        className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold shadow-lg shadow-emerald-200 active:scale-[0.98] transition-all rounded-2xl h-12"
                                    ...
                                    <div className="border-2 border-slate-100 rounded-2xl overflow-hidden bg-muted/10 group relative shadow-sm transition-all hover:shadow-md">
                                        ...
                                        <div className="absolute inset-0 bg-slate-900/40 opacity-0 group-hover:opacity-100 transition-all duration-300 flex items-center justify-center backdrop-blur-[2px]">
                                            <a
                                                ...
                                                className="bg-white text-slate-900 px-5 py-2.5 rounded-xl font-bold flex items-center shadow-xl hover:bg-white/90 active:scale-95 transition-all"
                                            >
                                    ...
                                    <div key={item.id} className="relative group animate-in fade-in slide-in-from-left-4 duration-500" style={{ animationDelay: `${index * 50}ms` }}>
                                        ...
                                        <div className="absolute -left-[37px] top-1.5 w-10 h-10 rounded-full border-4 border-white bg-white shadow-md flex items-center justify-center z-10 transition-transform group-hover:scale-110">
                                            <div className={`w-3 h-3 rounded-full ${
                                                ...
                                                item.action === 'comment' ? 'bg-indigo-500' :
                                                'bg-amber-500'
                                            } ring-4 ring-offset-0 ring-${item.action === 'created' ? 'emerald' : item.action === 'comment' ? 'indigo' : 'amber'}-500/10`}></div>
                                        </div>
                                        ...
                                        <div className="flex flex-col gap-2">
                                            <div className="flex flex-wrap items-center gap-x-3 text-xs">
                                                <span className={`font-bold px-2 py-0.5 rounded-full uppercase tracking-wider ${
                                                    ...
                                                    item.action === 'comment' ? 'bg-indigo-50 text-indigo-600' :
                                            ...
                                            <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm transition-all hover:shadow-md hover:border-slate-300">
                                                <div className="text-[15px] prose-sm">
                                                    ...
                                                    {item.comment && (
                                                        <div className="mt-3 bg-indigo-50/30 p-4 rounded-xl italic text-slate-700 border-l-4 border-indigo-400/50 flex gap-3">
                                                            <svg className="w-5 h-5 text-indigo-400 shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z" /></svg>
