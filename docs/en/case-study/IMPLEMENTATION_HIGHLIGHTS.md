...
### 12. My Workspace — Plan-Gated Module

#### Challenge
Providing a feature-rich personal workspace that is only accessible to tenants on plans that include it, without hardcoding plan IDs.

#### Solution
The `WorkspaceController` uses the `UsageEnforcement` trait to dynamically check whether the tenant's active plan includes the workspace feature before any operation is allowed.

**Key Implementation:**
```php
// WorkspaceController.php
public function index() {
    $this->enforceLimit('workspace');  // Checks plan feature flags
    // ... proceed to fetch workspace data
}
```

**Achievements:**
- ✅ Feature gating without hardcoded plan IDs
- ✅ Graceful upgrade prompt when plan doesn't include workspace
- ✅ Plan limits enforced at API boundary, not just UI

### 13. Quick Access — Frictionless Trial Provisioning

#### Challenge
Allowing prospects to try the product instantly without a full signup flow, while ensuring their trial is properly scoped and upgradeable.

#### Solution
The Quick Access flow auto-provisions a tenant account assigned to the platform's designated **trailing plan** (`is_trailing = true`). Only one plan can hold this designation at a time, enforced at the database level.

**Key Implementation Decisions:**
- `is_trailing` flag stored on the `plans` table.
- SA Package UI prevents deletion or un-designating the trailing plan while it is active.
- `is_public` flag controls which plans appear on the public pricing page independently.

### 14. Admin Wiki — Live Documentation Engine

#### Challenge
Keeping internal documentation in sync with the live application without requiring developers to manually update a separate documentation portal.

#### Solution
Built the Admin Wiki as a dynamic `.md` file reader. The `AdminWiki.php` controller scans the `docs/` directory at request time and returns a categorized file tree. The `SAWiki.tsx` frontend renders markdown with full GFM support, including tables (via `remark-gfm`) and Mermaid diagrams (via `mermaid` JS).

**Technical Highlights:**
- ✅ No static builds — file changes are reflected instantly
- ✅ Custom `code` renderer detects `mermaid` language and renders SVG diagrams
- ✅ Custom `a` renderer resolves relative `.md` links to in-wiki navigation
- ✅ Path sanitization in backend prevents directory traversal
