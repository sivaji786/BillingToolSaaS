...
The platform uses a dynamic subdomain routing system where each tenant accesses the application via `{tenant}.humpl.org`. 
...
graph TD
    ...
    E --> M[Line Items]
    E --> N[Tax Summary]
    E --> O[Validation Panel]

    F --> P[Search & Filters]
    F --> Q[Bulk Operations]
    F --> R[Import/Export]

    D --> S[Analytics Charts]
    D --> T[Recent Activity]

    A --> U[Admin Portal Router]
    U --> V[SA Dashboard]
    U --> W[SA Packages]
    U --> X[SA Tenants]
    U --> Y[SA Tickets]
...
```
POST   /admin/auth/login              # SA login
GET    /admin/tenants                  # List all tenants
GET    /admin/analytics/tenant-usage   # Usage breakdown
GET    /admin/packages                 # Plan management
GET    /admin/tickets                  # All support tickets
GET    /admin/wiki                     # Docs tree
GET    /admin/wiki/read               # Single doc content
```
