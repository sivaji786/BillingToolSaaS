...
sequenceDiagram
    ...
    Filter->>Filter: Resolve Tenant (from JWT TID -> ID 5)
    Filter->>Filter: Set config('App')->currentTenant = 5
    
    Filter->>RBAC: Check Rights
    RBAC->>RBAC: Verify Workspace (User TID matches ID 5)
...
2.  **Context**: 
    *   **Tenant**: Determined by `UnifiedAuthFilter` (from JWT).
    *   **User**: Determined by JWT Token (`userId`).
3.  **Insertion**:
