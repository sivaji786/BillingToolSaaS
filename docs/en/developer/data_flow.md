...
2.  **Unified Identification (`UnifiedAuthFilter`)**:
    ...
    *   **Method 3 (Subdomain)**: Checks the Host header.
    *   **Result**: 
        *   If valid: Sets `config('App')->currentTenant`.
        *   If invalid: Returns `404 Tenant Not Found` (unless it's a global public route).

### Phase 2: Authentication & Authorization
3.  **Authentication**:
    *   **UnifiedAuthFilter** also handles the user identification via JWT.
    *   **Action**: Sets `userId` in the request context and session.
4.  **RBAC & Scope Check (`RbacFilter`)**:
    *   **Permission Check**: Does the user have the required right (e.g., `invoices.read`)?
    *   **Workspace mismatch enclosure**: If the user's `tenant_id` does not match the workspace they are accessing (subdomain), they are redirected to their correct workspace.
    *   **Result**: If mismatch or lack of rights -> `403 Forbidden`.
...
### Frontend API Calls
Always use the helper service `src/services/api.ts`. It automatically injects the `Authorization` header and handles redirection if you access the wrong workspace. Note that `X-Tenant-ID` is no longer required as the backend resolves tenancy from the JWT token.
